...
+ [修复] 对象存储切换服务商后保留各自配置并支持整体导入导出，固定页面标题，同时修正深色主题开关滑块的颜色对比。
+ [新增] 对象存储支持火山云 TOS 与通用 S3 服务，复用直连上传、删除、列表和复制接口，支持服务商选择及通用 S3 寻址配置。
+ [修复] 素材审核弹窗隔离关闭等交互事件，避免触发画布操作，配置弹窗及渠道编辑窗口显示在审核弹窗上方。
+ [调整] 移除画布视频节点的手动参考图片地址和 Ark 参考视频来源输入，统一使用连接素材及其自动保存的引用信息。
+ [新增] 火山官方素材审核支持通过 CreateAssetGroup 创建虚拟人像素材组，创建成功后刷新列表并自动选中。
+ [调整] 火山官方素材审核通过 ListAssetGroups 下拉选择素材组，支持搜索、刷新和无素材组时的创建指引，避免手填错误 GroupId。
+ [调整] Seedance 图片与视频在生成前统一准备，保留火山来源的 30 天免审信息，其他素材通过公网 S3 副本审核并复用同渠道 asset:// 引用。

+ [调整] 对象存储统一上传、删除、列表和复制接口及返回结构，按云厂商拆分 S3 接入定义，便于扩展其他云存储。
+ [新增] 配置中心接入七牛云 S3 对象存储，支持直连上传、删除、列表和复制接口，以及配置导入导出与连接测试。
+ [修复] 可想AI素材审核查询改用创建响应的数字记录 ID，避免将 `asset-...` 素材 ID 作为整数路径参数导致校验失败。
+ [调整] 可想AI视频设置按各模型文档展示生成模式，使用用户所选 modelType 提交，并联动参考音视频入口。
+ [修复] 可想AI视频错误提示保留请求阶段、HTTP 状态和任务 ID，并读取接口返回的任务失败原因。
+ [新增] 增加可想AI渠道，支持文本、异步图片与视频生成，以及 Seedance 2.0/2.5 参考素材审核。
+ [调整] 可想AI使用独立渠道模块和默认图片、视频模型模板，分离原始展示名称与请求模型 ID，文本模型通过专用列表接口获取。
+ [修复] 可想AI模型选择保留请求 ID，素材审核使用当前渠道凭据，并按文档修正图片比例与 MiniMax、Kling Omni 请求参数。
+ [修复] Ark 视频展示名、无版本名称与官方 Model ID 共用 Seedance 参数配置，标准 API 和 Agent Plan 创建任务时统一映射实际调用 ID，并保留显式指定版本。
+ [修复] 火山方舟图片模型按官方 Model ID 映射展示名称，修复 Seedream 5.0 pro / lite 版本化模型的尺寸配置未命中和请求模型名错误。
+ [修复] 生图和视频工作台在提交时立即展示生成记录，生图记录保留逐张状态与失败原因并支持原位重试，切换记录不再被后台结果覆盖。
+ [新增] Ark 渠道支持标准 API、Agent Plan 接入方式，以及按渠道模型配置的视频生成参数、参考音视频和可恢复任务；已下线的 Agent Plan Seedance 1.5 Pro 会在提交前明确拒绝。
+ [修复] 图片尺寸选择与请求校验共用模型配置，按官方文档适配 Seedream 各模型的分辨率、比例预设和像素范围，修复 Ark 默认尺寸过小的问题。
+ [新增] 渠道支持独立 Ark（火山方舟）调用格式，提供水印、输出格式和提示词优化选项，并适配 Seedream 参考图编辑。
+ [调整] Web 开发命令默认同时启动本地 Canvas Proxy，并在退出时一并停止，简化跨域接口的本地调试。
+ [修复] OpenAI 兼容请求保留 Base URL 中显式指定的 API 版本路径，修复火山方舟 `/api/v3` 被重复追加 `/v1` 的问题。
