...
- 对象云存储使用独立的直连请求链，不读取或复用模型/全局本地代理配置，直连失败时也不得自动回退代理；七牛云通过 S3 兼容接口接入。
- 对象存储接口集中在 `web/src/services/api/s3/`，业务只调用统一的上传、删除、列表、复制接口；每个云厂商单独一个文件维护地区、域名等差异，共用 S3 实现与通用返回类型，不在业务组件散落厂商分支。
- 渠道专属的鉴权、接口路径、请求参数、任务查询和素材审核放在 `web/src/services/api/<渠道>/`；模型展示名称、请求 ID 与默认模板集中在对应渠道配置中维护，通用入口只按渠道分发，不散落具体模型映射和接口分支。
